...
class MessageBuilder {
			...
			if (bodyContent != null) {
				...
				if (bodyContent instanceof String) {
					...
					try {
						...
						throw new MessageException(PasserelleException.Severity.NON_FATAL,"",bodyContent,e);
                ...
                throw new MessageException("illegal content for XML",message.getBodyContentAsString(),e);
            }
		}
	...
	private class BodyPartElementBuilder {
		...
		public Element build(BodyPart part) throws IOException, MessagingException {
			...
			if (o instanceof String) {
				...
				/*				StreamContentBuilder builder = new StreamContentBuilder();
								if (part instanceof MimeBodyPart)
									bodyElement.addContent(builder.build(((MimeBodyPart) part).getRawInputStream()));
								else
	...
	public static ManagedMessage buildFromXML(String msgXML) throws MessageException {
		...
		try {
			...
			throw new MessageException(PasserelleException.Severity.NON_FATAL,"",msgXML,e);
		} catch (IOException e) {
			throw new MessageException(PasserelleException.Severity.NON_FATAL,"",msgXML,e);
		}
