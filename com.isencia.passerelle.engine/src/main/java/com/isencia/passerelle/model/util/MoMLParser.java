...
/**
 * This class constructs Ptolemy II models from specifications in MoML (modeling markup language), which is based on XML. The class contains an instance of the
 * Microstar &AElig;lfred XML parser and implements callback methods to interpret the parsed XML. The way to use this class is to call its parse() method. The
 * returned value is top-level composite entity of the model.
 * <p>
 * For convenience, there are several forms of the parse method. Most of these take two arguments, a base, and some specification of the MoML to parse (a stream
 * or the text itself). The base is used to interpret relative URLs that might be present in the MoML. For example, the base might be the document base of an
 * applet. An applet might use this class as follows:
 * 
 * <pre>
 ...
 * <p>
 * It can be difficult to create an appropriate URL to give as a base, particularly if what you have is a file or file name in the directory that you want to
 * use as a base. The easiest technique is to use the toURL() method of the File class. Some of the URL constructors, for reasons we don't understand, create
 * URLs that do not work.
 * <p>
 * The MoML code given to a parse() method may be a fragment, and does not need to include the "&lt;?xml ... &gt;" element nor the DOCTYPE specification.
 * However, if the DOCTYPE specification is not given, then the DTD will not be read. The main consequence of this, given the parser we are using, is that
 * default values for attributes will not be set. This could cause errors. The parser itself is not a validating parser, however, so it makes very limited use
 * of the DTD. This may change in the future, so it is best to give the DOCTYPE element.
 * <p>
 * The parse() methods can be used for incremental parsing. After creating an initial model using a call to parse(), further MoML fragments without top-level
 * entity or derived objects can be evaluated to modify the model. You can specify the context in which the MoML to be interpreted by calling setContext().
 * However, the XML parser limits each fragment to one element. So there always has to be one top-level element. If you wish to evaluate a group of MoML
 * elements in some context, set the context and then place your MoML elements within a group element, as follows:
 * 
 * <pre>
 *  &lt;group&gt
 ...
 * </pre>
 * 
 * The group element is ignored, and just serves to aggregate the MoML elements, unless it has a name attribute. If it has a name attribute, then the name
 * becomes a prefix (separated by a colon) of all the names of items immediately in the group element. If the value of the name attribute is "auto", then the
 * group is treated specially. Each item immediately contained by the group (i.e. not deeply contained) will be created with its specified name or a modified
 * version of that name that does not match a pre-existing object already contained by the container. That is, when name="auto" is specified, each item is
 * forced to be created with unique name, rather than possibly matching a pre-existing item.
 * <p>
 ...
 * <p>
 * This parser supports the way Ptolemy II handles hierarchical models, where components are instances cloned from reference models called "classes." A model (a
 * composite entity) is a "class" in Ptolemy II if the elementName field of its MoMLInfo object is the string "class". If a component is cloned from a class,
 * then when that component exports MoML, it references the class from which it was cloned and exports only differences from that class. I.e., if further
 * changes are made to the component, it is important that when the component exports MoML, that those changes are represented in the exported MoML. This
 * effectively implements an inheritance mechanism, where a component inherits all the features of the master from which it is cloned, but then extends the
 * model with its own changes.
 * <p>
 ...
 * <p>
 * This class works closely with MoMLChangeRequest to implement another feature of Ptolemy II hierarchy. In particular, if an entity is cloned from another that
 * identifies itself as a "class", then any changes that are made to the class via a MoMLChangeRequest are also made to the clone. This parser ensures that
 * those changes are <i>not</i> exported when MoML is exported by the clone, because they will be exported when the master exports MoML.
 * 
...
public class MoMLParser extends ptolemy.moml.MoMLParser {
  ...
  /**
   * Construct a parser that creates entities in the specified workspace. If the argument is null, create a new workspace with an empty name. Classes will be
  ...
  /**
   * Construct a parser that creates entities in the specified workspace. If the workspace argument is null, then create a new workspace with an empty name.
   * Classes will be created using the classloader that created this class.
   * 
   * @param workspace The workspace into which to place entities.
   * @param loader The class loader that will be used to create classes, or null if the the bootstrap class loader is to be used.
   */
  ...
  /**
   * Add a MoMLFilter to the end of the list of MoMLFilters used to translate names. If the list of MoMLFilters already contains the filter, then the filter is
   * not added again. Note that this method is static. The specified MoMLFilter will filter all MoML for any instances of this class.
   * 
  ...
  /**
   * Add a List of MoMLFilters to the end of the list of MoMLFilters used to translate names. The argument list of filters is added even if the current list
   * already contains some of the filters in the argument list. Note that this method is static. The specified MoMLFilter will filter all MoML for any instances
   * of this class.
   * 
   * @param filterList The list of MoMLFilters to add to the list of MoMLFilters to be used to translate names.
  ...
  /**
   * Handle an attribute assignment that is part of an XML element. This method is called prior to the corresponding startElement() call, so it simply
   * accumulates attributes in a hashtable for use by startElement().
   * 
   * @param name The name of the attribute.
   * @param value The value of the attribute, or null if the attribute is <code>#IMPLIED</code> and not specified.
   * @param specified True if the value is specified, false if the value comes from the default value in the DTD rather than from the XML file.
  ...
  public void attribute(String name, String value, boolean specified) throws XmlException {
    ...
    if (_namespace.equals(_AUTO_NAMESPACE) && (_current != null)
        && (name.equals("name") || name.equals("port") || name.startsWith("relation") || name.equals("vertex") || name.equals("pathTo"))) {
      ...
      if (!_namespace.equals(_DEFAULT_NAMESPACE) && !_namespace.equals(_AUTO_NAMESPACE)
          && (name.equals("name") || name.equals("port") || name.equals("relation") || name.equals("vertex") || name.equals("pathTo"))) {
   ...
   * 
   * @param change The change that has been executed, or null if the change was not done via a ChangeRequest.
   */
  public void changeExecuted(ChangeRequest change) {
  }

  /**
   * React to a change request has resulted in an exception. This method is called after a change request was executed, but during the execution an exception
   * was thrown. Presumably the change was a propagation request. If there is a registered error handler, then the error is delegated to that handler.
   * Otherwise, the error is reported to stderr.
   * 
   * @param change The change that was attempted or null if the change was not done via a ChangeRequest.
   * @param exception The exception that resulted.
   */
  public void changeFailed(ChangeRequest change, Exception exception) {
    if (_handler != null) {
      int reply = _handler.handleError(change.toString(), _toplevel, exception);
  ...
  /**
   * Handle character data. In this implementation, the character data is accumulated in a buffer until the end element. Character data appears only in doc and
   * configure elements. &AElig;lfred will call this method once for each chunk of character data found in the contents of elements. Note that the parser may
   * break up a long sequence of characters into smaller chunks and call this method once for each chunk.
   * 
  ...
  /**
   * If a public ID is given, and is not that of MoML, then throw a CancelException, which causes the parse to abort and return null. Note that the version
   * number is not checked, so future versions of MoML should also work.
   * 
  ...
  /**
   * End the document. The MoMLParser calls this method once, when it has finished parsing the complete XML document. It is guaranteed that this will be the
   * last method called in the XML parsing process. As a consequence, it is guaranteed that all dependencies between parameters used in the XML description are
   * resolved. This method executes any change requests that may have been made during the parsing process.
   * 
   * @exception CancelException If an error occurs parsing one of the parameter values, and the user clicks on "cancel" to cancel the parse.
   */
    ...
    try {
      ...
      while (parameters.hasNext()) {
        ...
        try {
          ...
          if (_handler != null) {
            int reply = _handler.handleError("<param name=\"" + param.getName() + "\" value=\"" + param.getExpression() + "\"/>", param.getContainer(), ex);
  ...
  /**
   * End an element. This method pops the current container from the stack, if appropriate, and also adds specialized properties to the container, such as
   * <i>_doc</i>, if appropriate. &AElig;lfred will call this method at the end of each element (including EMPTY elements).
   * 
  ...
  public void endElement(String elementName) throws Exception {
    ...
    if (_skipElement <= 0) {
      ...
      if (elementName.equals("configure")) {
        ...
        if (_configureNesting < 0) {
          throw new XmlException("Internal Error: _configureNesting is " + _configureNesting + " which is <0, which indicates a nesting bug",
        ...
        if (_docNesting < 0) {
          throw new XmlException("Internal Error: _docNesting is " + _docNesting + " which is <0, which indicates a nesting bug", _currentExternalEntity(),
    ...
    if (_skipRendition) {
      ...
      } else if (elementName.equals("property") || elementName.equals("director") || elementName.equals("port") || elementName.equals("relation")
          || elementName.equals("rendition") || elementName.equals("vertex")) {
        try {
          _current = (NamedObj) _containers.pop();
        } catch (EmptyStackException ex) {
          // We are back at the top level.
  ...
  /**
   * Indicate a fatal XML parsing error. &AElig;lfred will call this method whenever it encounters a serious error. This method simply throws an XmlException.
   * 
  ...
  /**
   * Given a file name or URL description, find a URL on which openStream() will work. If a base is given, the URL can be found relative to that base. If the
   * URL cannot be found relative to this base, then it is searched for relative to the current working directory (if this is permitted with the current
   * security restrictions), and then relative to the classpath. If the URL is external and is not relative to a previously defined base, then the user will be
   * warned about a security concern and given the opportunity to cancel.
   * <p>
   ...
   * @param base The base URL for relative references, or null if there is none.
   * @return A URL on which openStream() will succeed.
   * @exception Exception If the file or URL cannot be found or if the user cancels on being warned of a security concern.
   */
  public URL fileNameToURL(String source, URL base) throws Exception {
    ...
    try {
      ...
      if ((protocol != null) && protocol.trim().toLowerCase().equals("http")) {
        ...
        if (((security == null) || (withinUntrustedApplet == false)) && !_approvedRemoteXmlFiles.contains(result)) {
          // If the result and _base have a common root,
          // then the code is ok.
          String resultBase = result.toString().substring(0, result.toString().lastIndexOf("/"));

          if ((_base == null) || !resultBase.startsWith(_base.toString())) {
    ...
    if (input == null) {
      throw new XmlException(errorMessage.toString(), _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
  ...
  /**
   * Get the error handler to handle parsing errors. Note that this method is static. The returned error handler will handle all errors for any instance of this
   * class.
   * 
  ...
  /**
   * Get the List of MoMLFilters used to translate names. Note that this method is static. The returned MoMLFilters will filter all MoML for any instances of
   * this class.
   * 
  ...
  /**
   * Parse the MoML file at the given URL, which may be a file on the local file system, using the specified base to expand any relative references within the
   * MoML file. If the <i>input</i> URL has already been parsed, then return the model that previously parsed. Note that this means that an application that
   * opens and then closes a model and expects to re-parse the XML when re-opening must call purgeModelRecord() when closing it. This method uses parse(URL,
   * InputStream).
   * 
   * @param base The base URL for relative references, or null if not known.
   * @param input The stream from which to read XML.
   * @return The top-level composite entity of the Ptolemy II model, or null if the file is not recognized as a MoML file.
  ...
  /**
   * Parse the given stream, using the specified url as the base to expand any external references within the MoML file. This method uses parse(URL, Reader).
   * Note that this bypasses the mechanism of parse(URL, URL) that returns a previously parsed model. This method will always re-parse using data from the
   * stream.
   * 
   * @param base The base URL for relative references, or null if not known.
   * @param input The stream from which to read XML.
  ...
  /**
   * Parse the given stream, using the specified url as the base The reader is wrapped in a BufferedReader before being used. Note that this bypasses the
   * mechanism of parse(URL, URL) that returns a previously parsed model. This method will always re-parse using data from the stream.
   * 
   * @param base The base URL for relative references, or null if not known.
  ...
  public NamedObj parse(URL base, Reader reader) throws Exception {
    ...
    if (parser != this) {
      ...
      ParserAttribute parserAttribute = (ParserAttribute) _toplevel.getAttribute("_parser", ParserAttribute.class);
  ...
  /**
   * Parse the given string, which contains MoML. If there are external references in the MoML, they are interpreted relative to the current working directory.
   * Note that this method attempts to read the user.dir system property, which is not generally available in applets. Hence it is probably not a good idea to
   * use this method in applet code, since it will probably fail outright.
   * 
   * @param text The string from which to read MoML.
   * @return The top-level composite entity of the Ptolemy II model.
   * @exception Exception If the parser fails.
   * @exception SecurityException If the user.dir system property is not available.
   */
  ...
  /**
   * Parse the file with the given name, which contains MoML. If there are external references in the MoML, they are interpreted relative to the current working
   * directory.
   * <p>
   ...
   * use this method in applet code, since it will probably fail outright.
   * <p>
   * If the file has already been parsed, then return the model that previously parsed. Note that this means that an application that opens and then closes a
   * model and expects to re-parse the XML when re-opening should call purgeModelRecord() when closing it.
   * 
  ...
  /**
   * Handle a processing instruction. Processing instructions are allowed in doc and configure elements, and are passed through unchanged. In the case of the
   * doc element, they will be stored in the Documentation attribute. In the case of the configure element, they will be passed to the configure() method of the
   * parent object.
   * 
  ...
  /**
   * Purge any record of a model opened from the specified URL. The record will not be purged if the model is a class definition that has child instances. Note
   * that you may also need to call {@link #reset()} so that the _toplevel is reset on any parser.
   * 
   * @param url The URL.
   * @see #parse(URL, URL)
   * @see #purgeAllModelRecords()
   ...
   * use this method in applet code, since it will probably fail outright.
   * <p>
   * Note that you may also need to call {@link #reset()} so that the _toplevel is reset.
   * 
   * @param filename The file name from which to read MoML.
  ...
  /**
   * Resolve an external entity. If the first argument is the name of the MoML PUBLIC DTD ("-//UC Berkeley//DTD MoML 1//EN"), then return a StringReader that
   * will read the locally cached version of this DTD (the public variable MoML_DTD_1). Otherwise, return null, which has the effect of deferring to
   * &AElig;lfred for resolution of the URI. Derived classes may return a a modified URI (a string), an InputStream, or a Reader. In the latter two cases, the
   * input character stream is provided.
   * 
  ...
  /**
   * Given the name of a MoML class and a source URL, check to see whether this class has already been instantiated, and if so, return the previous instance. If
   * the source is non-null, then this finds an instance that has been previously opened by this application from the same URL. If the source is null and the
   * class name is absolute (starting with a period), then look for the class in the current top level. If the source is null and the class name is relative,
   * then look for the class relative to the current context.
   * 
   * @param name The name of the MoML class to search for.
   * @param source The URL source
   * @return If the class has already been instantiated, return the previous instance, otherwise return null.
    ...
    if ((_imports != null) && (source != null)) {
      ...
      if (possibleCandidate instanceof ComponentEntity) {
        ...
        if (candidate.isClassDefinition()) {
          ...
          if ((lastPeriod >= 0) && (candidateClassName.length() > (lastPeriod + 1))) {
  ...
  /**
   * Set the context for parsing. This can be used to associate this parser with a pre-existing model, which can then be modified via incremental parsing. This
   * calls reset() and sets the top-level entity to the top-level of the specified object.
   * <p>
   * Callers should be careful about calling this method and resetting the modified flag to false when parsing moml that has been modified by the backward
   * compatibility filter. It is safer to do something like:
   * 
   * <pre>
  ...
  /**
   * Set the list of MoMLFilters used to translate names. Note that this method is static. The specified MoMLFilters will filter all MoML for any instances of
   * this class.
   * 
  ...
  /**
   * Record whether the parsing of the moml modified the data. If a MoMLFilter modifies the model by returning a different value, then the MoMLFilter should
   * call this method with a true argument.
   * 
  ...
  /**
   * Set the top-level entity. This can be used to associate this parser with a pre-existing model, which can then be modified via incremental parsing. This
   * calls reset().
   * 
  ...
  /**
   * Start a document. This method is called just before the parser attempts to read the first entity (the root of the document). It is guaranteed that this
   * will be the first method called. In this implementation, this method resets some private variables, and if there is a top level model associated with this
   * parser, sets it so that change requests are deferred rather than executed. The change requests will be executed as a batch in endDocument().
   * 
   * @see #endDocument()
   */
  ...
  /**
   * Start an element. This is called at the beginning of each XML element. By the time it is called, all of the attributes for the element will already have
   * been reported using the attribute() method. Unrecognized elements are ignored.
   * 
   * @param elementName The element type name.
   * @exception XmlException If the element produces an error in constructing the model.
   */
    ...
    try {
      ...
      if (elementName.equals("class")) {
        ...
        if (!existedAlready) {
          ...
          NamedObj candidate = _createEntity(className, versionSpec, entityName, source);

          if (candidate instanceof Entity) {
            entity = (Entity) candidate;
          } else {
            throw new IllegalActionException(_current, "Attempt to create a class named " + entityName + " from a class that "
        ...
        if (_current != null) {
          ...
          if (_xmlFile != null) {
            ...
            URIAttribute attribute = new URIAttribute(_toplevel, "_uri");
        ...
        _checkClass(_current, Configurable.class, "Element \"configure\" found inside an element that " + "does not implement Configurable. It is: " + _current);
        ...
        String entityName = (String) _attributes.get("name");
        _checkForNull(entityName, "No name for element \"deleteEntity\"");
        ...
        _checkForNull(propName, "No name for element \"deleteProperty\"");
        ...
        _checkForNull(relationName, "No name for element \"deleteRelation\"");
        ...
        _checkClass(_current, CompositeActor.class, "Element \"director\" found inside an element that " + "is not a CompositeActor. It is: " + _current);
        ...
        String version = (String) _attributes.get("version");
        VersionSpecification versionSpec = buildVersionSpecification(version);
        Class newClass = loadClass(className, versionSpec);
        ...
        if (_current != null) {
          ...
          if (displayName != null) {
            ...
            if (_undoEnabled) {
              ...
              _undoContext.appendUndoMoML("<display name=\"" + StringUtilities.escapeForXML(oldDisplayName) + "\"/>\n");
        ...
        // ////////////////////////////////////////////////////////////
        // // entity
      } else if (elementName.equals("entity") || elementName.equals("model")) {
        ...
        if (existedAlready) {
          ...
          if (candidate instanceof Entity) {
            ...
            throw new IllegalActionException(_current, "Attempt to create an entity named " + entityName + " from a class that "
                + "is not a subclass of Entity: " + className);
          }
        }
        ...
        String insertInsideAtSpec = (String) _attributes.get("insertInsideAt");
        ...
        _checkClass(_current, Entity.class, "Element \"port\" found inside an element that " + "is not an Entity. It is: " + _current);
        ...
        if (port != null) {
          if (newClass != null) {
            ...
            _checkClass(port, newClass, "port named \"" + portName + "\" exists and is not an instance of " + className);
          }
        } else {
          ...
          while (derivedObjects.hasNext()) {
            ...
            if (derived.getPort(portName) != null) {
              throw new IllegalActionException(container, "Cannot create port because a subclass or instance " + "contains a port with the same name: "
        ...
        if (_undoEnabled) {
          if (alreadyExisted) {
            ...
            _undoContext.appendUndoMoML("<deletePort name=\"" + portName + "\" />\n");
        ...
        if (port instanceof IOPort) {
          ...
          if (direction != null) {
            ...
            boolean isOutput = direction.equals("output") || direction.equals("both");
            boolean isInput = direction.equals("input") || direction.equals("both");
            ...
            if (alreadyExisted && (ioport.getDerivedLevel() < Integer.MAX_VALUE)) {
              if ((ioport.isInput() != isInput) || (ioport.isOutput() != isOutput)) {
                throw new IllegalActionException(ioport, "Cannot change whether this port is " + "an input or output. That property is "
        ...
        if (createIfNecessary != null && createIfNecessary.equals("true") && _current != null && propertyName != null
        ...
        _checkClass(_current, CompositeEntity.class, "Element \"relation\" found inside an element that " + "is not a CompositeEntity. It is: " + _current);
        ...
        if (relation == null) {
          ...
          if (newClass != null) {
            _checkClass(relation, newClass, "relation named \"" + relationName + "\" exists and is not an instance of " + className);
          }
        ...
        if (_undoEnabled) {
          if (alreadyExisted) {
            ...
            _undoContext.appendUndoMoML("<relation name=\"" + relationName + "\" ");
            ...
            _undoContext.appendUndoMoML("<deleteRelation name=\"" + relationName + "\" />\n");
        ...
        if (_current != null) {
          ...
          if (!oldName.equals(newName) && (_current.getDerivedLevel() < Integer.MAX_VALUE)) {
            throw new IllegalActionException(_current, "Cannot change the name to " + newName + ". The name is fixed by the class definition.");
          }
          ...
          try {
            while (derivedObjects.hasNext()) {
              ...
              if (derived instanceof Instantiable) {
                ...
                if ((parent != null) && ((parent == _current) || changedName.contains(parent))) {
                  ...
                  if (last < 0) {
                    throw new InternalErrorException("Expected instance " + derived.getFullName() + " to have class name ending with " + oldName
                        + " but its class name is " + previousClassName);
                  }
            ...
            throw new IllegalActionException(_current, ex, "Propagation to instance and/or derived class causes" + "name duplication: " + derived.getFullName());
          }
        ...
        _checkClass(_current, Relation.class, "Element \"vertex\" found inside an element that " + "is not a Relation. It is: " + _current);
      ...
      // No handler.
      throw new XmlException("XML element \"" + elementName + "\" triggers exception:\n  " + ex.getTargetException(), _currentExternalEntity(),
          _getLineNumber(), _getColumnNumber(), ex.getTargetException());
    } catch (Exception ex) {
      exceptionThrown = true;
      ...
      if (ex instanceof XmlException) {
        ...
        throw new XmlException("XML element \"" + elementName + "\" triggers exception.", _currentExternalEntity(), _getLineNumber(), _getColumnNumber(), ex);
      }
    } finally {
      _attributes.clear();
      _attributeNameList.clear();
   ...
   * 
   * @return The list of top objects created since clearTopObjectsList() was called, or null if it has not been called.
  ...
  /**
   * The standard MoML DTD, represented as a string. This is used to parse MoML data when a compatible PUBLIC DTD is specified. NOTE: This DTD includes a number
   * of elements that are deprecated. They are included here for backward compatibility. See the MoML chapter of the Ptolemy II design document for a view of
   * the current (nondeprecated) DTD.
   */
  ...
  /**
   * List of Strings that name files to be skipped. This variable is used primarily for testing configurations. The value of this variable is a List of Strings,
   * where each element names a file name that should _not_ be loaded if it is encounted in an input statement.
   */
   ...
   * 
   * @return A string giving the URI of the external entity being read, or null if none.
   */
   ...
   */
  private ComponentEntity _attemptToFindMoMLClass(String className, VersionSpecification versionSpec, String source) throws Exception {
    ...
    try {
      ...
      if (altClassAsFile != null) {
        ...
        try {
          ...
          // both were.
          throw new XmlException("Could not find '" + classAsFile + "' or '" + altClassAsFile + "' using base '" + _base + "': ", _currentExternalEntity(),
    ...
    if (candidateReference instanceof ComponentEntity) {
      ...
      throw new XmlException("File " + classAsFile + " does not define a ComponentEntity.", _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
    ...
    if (!className.equals(referenceName) && !className.endsWith("." + referenceName)) {
      ...
      if (reference == null) {
        throw new XmlException("File " + classAsFile + " does not define a class named " + className, _currentExternalEntity(), _getLineNumber(),
            _getColumnNumber());
      }
    }
  ...
  private void _checkClass(Object object, Class correctClass, String msg) throws XmlException {
    if (!correctClass.isInstance(object)) {
      throw new XmlException(msg, _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
  }
  ...
  private void _checkForNull(Object object, String message) throws XmlException {
    if (object == null) {
      throw new XmlException(message, _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
  }
   ...
   * @exception Exception
   */
  private NamedObj _createEntity(String className, VersionSpecification versionSpec, String entityName, String source) throws Exception {
    if ((_current != null) && !(_current instanceof CompositeEntity)) {
      throw new XmlException("Cannot create an entity inside " + "of another that is not a CompositeEntity " + "(Container is '" + _current + "').",
          _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
    ...
    if (reference == null) {
      ...
      if (_current != null) {
        ...
        while (derivedObjects.hasNext()) {
          ...
          if (derived.getEntity(entityName) != null) {
            throw new IllegalActionException(container, "Cannot create entity because a subclass or instance " + "contains an entity with the same name: "
                + derived.getEntity(entityName).getFullName());
          }
        }

        _checkClass(_current, CompositeEntity.class, "Cannot create an entity inside an element that " + "is not a CompositeEntity. It is: " + _current);
      ...
      if (!reference.isClassDefinition()) {
        throw new MissingClassException("Attempt to extend an entity that " + "is not a class: " + reference.getFullName(), reference.getFullName(),
            _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
      }
      ...
      URIAttribute modelURI = (URIAttribute) newEntity.getAttribute("_uri", URIAttribute.class);
   ...
   * @exception Exception If no matching constructor is found, or if invoking the constructor triggers an exception.
   */
  private NamedObj _createInstance(Class newClass, Object[] arguments) throws Exception {
    ...
    for (int i = 0; i < constructors.length; i++) {
      ...
      if (match) {
        ...
        // If we are keeping track of objects created...
        if ((_topObjectsCreated != null) && (arguments[0] == _originalContext)) {
          _topObjectsCreated.add(newEntity);
        }
    ...
    throw new XmlException("Cannot find a suitable constructor (" + arguments.length + " args) (" + argumentBuffer + ") for '" + newClass.getName() + "'",
        _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
  }

  /**
   * Delete the entity after verifying that it is contained (deeply) by the current environment. If no object is found, then do nothing and return null. This is
   * because deletion of a class may result in deletion of other objects that make this particular delete call redundant.
   * 
   ...
   * @exception Exception If there is no such entity or if the entity is defined in the class definition.
   */
  private NamedObj _deleteEntity(String entityName) throws Exception {
    ...
    if (toDelete.getDerivedLevel() < Integer.MAX_VALUE) {
      throw new IllegalActionException(toDelete, "Cannot delete. This entity is part of the class definition.");
    }
  ...
  /**
   * Delete the port after verifying that it is contained (deeply) by the current environment. If no object is found, then do nothing and return null. This is
   * because deletion of a class may result in deletion of other objects that make this particular delete call redundant.
   * 
   * @param portName The relative or absolute name of the port to delete.
   * @param entityName Optional name of the entity that contains the port (or null to use the current context).
   * @return The deleted object, or null if none is found.
   * @exception Exception If there is no such port or if the port is defined in the class definition.
   */
  private Port _deletePort(String portName, String entityName) throws Exception {
    ...
    if (portContainer == null) {
      throw new XmlException("No container for the port: " + portName, _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }

    // Ensure that derived objects aren't changed.
    if (toDelete.getDerivedLevel() < Integer.MAX_VALUE) {
      throw new IllegalActionException(toDelete, "Cannot delete. This port is part of the class definition.");
    }
  ...
  /**
   * Delete an attribute after verifying that it is contained (deeply) by the current environment. If no object is found, then do nothing and return null. This
   * is because deletion of a class may result in deletion of other objects that make this particular delete call redundant.
   * 
   * @param attributeName The relative or absolute name of the attribute to delete.
   * @return The deleted object, or null if none is found.
   * @exception Exception If there is no such attribute or if the attribute is defined in the class definition.
   */
  private Attribute _deleteProperty(String attributeName) throws Exception {
    ...
    // Ensure that derived objects aren't changed.
    if (toDelete.getDerivedLevel() < Integer.MAX_VALUE) {
      throw new IllegalActionException(toDelete, "Cannot delete. This attribute is part of the class definition.");
    }
  ...
  /**
   * Delete the relation after verifying that it is contained (deeply) by the current environment. If no object is found, then do nothing and return null. This
   * is because deletion of a class may result in deletion of other objects that make this particular delete call redundant.
   * 
   * @param relationName The relative or absolute name of the relation to delete.
   * @return The deleted object, or null if none is found.
   * @exception Exception If there is no such relation or if the relation is defined in the class definition.
   */
  private Relation _deleteRelation(String relationName) throws Exception {
    ...
    if (toDelete.getDerivedLevel() < Integer.MAX_VALUE) {
      throw new IllegalActionException(toDelete, "Cannot delete. This relation is part of the class definition.");
    }
    ...
    try {
      ...
      while (derivedObjects.hasNext()) {
        ComponentRelation derived = (ComponentRelation) derivedObjects.next();
  ...
  /**
   * Use the specified parser to parse the file or URL, which contains MoML, using the specified base to find the URL. If the URL has been previously parsed by
   * this application, then return the instance that was the result of the previous parse. If the URL cannot be found relative to this base, then it is searched
   * for relative to the current working directory (if this is permitted with the current security restrictions), and then relative to the classpath.
   * 
   ...
   * @param source The source file to assign if the file is parsed anew, or null to not assign one.
   * @return The top-level composite entity of the Ptolemy II model.
   * @exception Exception If the parser fails.
   */
   ...
   * 
   * @return the column number from the XmlParser. Return -1 if _xmlParser is null.
   */
   ...
   * 
   * @return the line number from the XmlParser. Return -1 if _xmlParser is null.
   */
  ...
  /**
   * Return the port corresponding to the specified port name in the specified composite entity. If the port belongs directly to the composite entity, then the
   * argument is a simple name. If the port belongs to a component entity, then the name is the entity name, a period, and the port name. Throw an exception if
   * there is no such port. The returned value is never null.
   * 
  ...
  private ComponentPort _getPort(String portspec, CompositeEntity context) throws XmlException {
    ...
    _checkForNull(port, "No port named \"" + portspec + "\" in " + context.getFullName());
    return port;
  }
  ...
  private String _getUndoForDeleteAttribute(Attribute toDelete) throws IOException {
    ...
    StringBuffer moml = new StringBuffer(UndoContext.moveContextStart(_current, toDelete));
  ...
  /**
   * Return the MoML commands to undo deleting the specified entity from the current context. Unless the container implements the marker interface
   * HandlesInternalLinks, this will generate undo MoML that takes care of re-creating any links that get broken as a result of deleting the specified entity.
   * If both ends of the link are defined in the base class, however, then the link is not reported.
   * 
   * @param toDelete The component to delete.
   */
  ...
  /**
   * Return the MoML commands to undo deleting the specified port from the current context. Unless the container implements the marker interface
   * HandlesInternalLinks, this will generate undo MoML that takes care of re-creating any inside links that get broken as a result of deleting the specified
   * port. Unless the container of the container implements HandlesInternalLinks, this will also generate undo to recreate the outside links.
   * 
   * @param toDelete The component to delete.
   */
  private String _getUndoForDeletePort(Port toDelete) throws IOException {
    ...
    if (container != null) {
      ...
      if (containerContainer instanceof CompositeEntity && !(containerContainer instanceof HandlesInternalLinks)) {
  ...
  /**
   * Return the MoML commands to undo deleting the specified relation from the current context. Unless the container implements the marker interface
   * HandlesInternalLinks, this will generate undo MoML that takes care of re-creating any links that get broken as a result of deleting the specified relation.
   * 
   * @param toDelete The component to delete.
   */
   ...
   */
  private void _handlePropertyElement(String className, String propertyName, final String value) throws Exception {
    ...
    if (propertyName.equals("multiport") && isIOPort) {
      ...
      // If this object is a derived object, then its I/O status
      // cannot be changed.
      if ((_current.getDerivedLevel() < Integer.MAX_VALUE) && (((IOPort) _current).isMultiport() != newValue)) {
        throw new IllegalActionException(_current, "Cannot change whether this port is " + "a multiport. That property is fixed by " + "the class definition.");
      }
      ...
      // Handle undo
      if (_undoEnabled) {
        _undoContext.appendUndoMoML("<property name=\"" + propertyName + "\" value=\"");
      ...
      // If this object is a derived object, then its I/O status
      // cannot be changed.
      if ((_current.getDerivedLevel() < Integer.MAX_VALUE) && (((IOPort) _current).isOutput() != newValue)) {
        throw new IllegalActionException(_current, "Cannot change whether this port is " + "an output. That property is fixed by " + "the class definition.");
      }
      ...
      // If this object is a derived object, then its I/O status
      // cannot be changed.
      if ((_current.getDerivedLevel() < Integer.MAX_VALUE) && (((IOPort) _current).isInput() != newValue)) {
        throw new IllegalActionException(_current, "Cannot change whether this port is " + "an input. That property is fixed by " + "the class definition.");
      }
      ...
      if (className != null) {
        try {
          ...
          throw new XmlException("Failed to find class '" + className + "'", _currentExternalEntity(), _getLineNumber(), _getColumnNumber(), ex);
          ...
          if (_current == null) {
            ...
            while (derivedObjects.hasNext()) {
              ...
              if ((other != null) && !(other instanceof Singleton)) {
                throw new IllegalActionException(_current, "Cannot create attribute because a subclass or instance "
                    + "contains an attribute with the same name: " + derived.getAttribute(propertyName).getFullName());
              }
            }
            ...
            if (!(property instanceof Attribute)) {
              ...
              throw new XmlException("Property is not an " + "instance of Attribute. ", _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
            }
      ...
      if (!createdNew) {
        ...
        if (value != null) {
          if (!(property instanceof Settable)) {
            throw new XmlException("Property is not an " + "instance of Settable, " + "so can't set the value.", _currentExternalEntity(), _getLineNumber(),
                _getColumnNumber());
          }
      ...
      if (_undoEnabled) {
        if (!previouslyExisted) {
          ...
          _undoContext.appendUndoMoML("<deleteProperty name=\"" + propertyName + "\" />\n");
          ...
          _undoContext.appendUndoMoML("<property name=\"" + property.getName() + "\" ");
          ...
          if (oldValue != null) {
            ...
            // try undo.
            _undoContext.appendUndoMoML("value=\"" + StringUtilities.escapeForXML(oldValue) + "\" ");
          }
  ...
  /**
   * Return true if the link between the specified port and relation is part of the class definition. It is part of the class definition if either the port and
   * the relation are at the same level of hierarchy and are both derived objects, or if the relation and the container of the port are both class elements. If
   * the relation is null, then this return true if the port and its container are derived objects. NOTE: This is not perfect, since a link could have been
  ...
  private boolean _isLinkInClass(NamedObj context, Port port, Relation relation) {
    boolean portIsInClass = (port.getContainer() == context) ? (port.getDerivedLevel() < Integer.MAX_VALUE)
  ...
  /**
   * Return true if the link between the specified relations is part of the class definition. It is part of the class definition if both are derived objects.
   * NOTE: This is not perfect, since a link could have been created between these elements in a subclass.
   * 
   * @param context The context containing the link.
  ...
  /**
   * Return whether or not the given element name is undoable. NOTE: we need this method as the list of actions on namespaces and _current does not apply to
  ...
  private boolean _isUndoableElement(String elementName) {
    ...
    if (elementName.equals("property") || elementName.equals("class") || elementName.equals("doc") || elementName.equals("deleteEntity")
        || elementName.equals("deletePort") || elementName.equals("deleteProperty") || elementName.equals("deleteRelation") || elementName.equals("display")
        || elementName.equals("entity") || elementName.equals("group") || elementName.equals("link") || elementName.equals("port")
        || elementName.equals("relation") || elementName.equals("rename") || elementName.equals("unlink") || elementName.equals("vertex")) {
      return true;
    }
  ...
  /**
   * If the file with the specified name exists, parse it in the context of the specified instance. If it does not exist, do nothing. If the file creates new
   * objects, then mark those objects as a class element with the same depth as the context.
   * 
   * @param fileName The file name.
   * @param context The context into which to load the file.
   * @return True if a file was found.
   * @exception Exception If the file exists but cannot be read for some reason.
   */
...
/** Look for a MoML file associated with the specified class
  ...
  /**
   * Mark deeply contained parameters as needing validation. This method adds all (deeply) contained instances of Settable to the _paramsToParse list, which
   * ensures that they will be validated.
   * 
  ...
  /**
   * Use the specified parser to parse the file or URL, which contains MoML, using the specified base to find the URL. If the URL cannot be found relative to
   * this base, then it is searched for relative to the current working directory (if this is permitted with the current security restrictions), and then
  ...
  private void _processLink(String relation1Name, String relation2Name) throws XmlException, IllegalActionException {
    _checkClass(_current, CompositeEntity.class, "Element \"link\" found inside an element that " + "is not a CompositeEntity. It is: " + _current);
    ...
    _checkForNull(relation1, "No relation named \"" + relation1Name + "\" in " + context.getFullName());

    // Get relations.
    ComponentRelation relation2 = context.getRelation(relation2Name);
    _checkForNull(relation2, "No relation named \"" + relation2Name + "\" in " + context.getFullName());
    ...
    if (_isLinkInClass(context, relation1, relation2)) {
      throw new IllegalActionException(relation1, relation2, "Cannot link relations when both" + " are part of the class definition.");
    }
    ...
    while (derivedObjects.hasNext()) {
      ComponentRelation derivedRelation1 = (ComponentRelation) derivedObjects.next();
    ...
    if (_undoEnabled) {
      ...
      if (relation1.getDerivedLevel() == Integer.MAX_VALUE || relation2.getDerivedLevel() == Integer.MAX_VALUE) {
        // Enclose in a group to prevent deferral on undo.
        _undoContext.appendUndoMoML("<group><unlink relation1=\"" + relation1Name + "\" relation2=\"" + relation2Name + "\" /></group>\n");
      }
    }
  }
  ...
  private void _processLink(String portName, String relationName, String insertAtSpec, String insertInsideAtSpec) throws XmlException, IllegalActionException {
    ...
    if (countArgs == 0) {
      throw new XmlException("Element link requires at least one of " + "an insertAt, an insertInsideAt, or a relation.", _currentExternalEntity(),
          _getLineNumber(), _getColumnNumber());
    }

    if ((insertAtSpec != null) && (insertInsideAtSpec != null)) {
      throw new XmlException("Element link requires at most one of " + "insertAt and insertInsideAt, not both.", _currentExternalEntity(), _getLineNumber(),
          _getColumnNumber());
    }
    ...
    if (relationName != null) {
      ...
      _checkForNull(tmpRelation, "No relation named \"" + relationName + "\" in " + context.getFullName());
    ...
    if (_isLinkInClass(context, port, relation)) {
      throw new IllegalActionException(port, "Cannot link a port to a relation when both" + " are part of the class definition.");
    }
    ...
    if (relation != null) {
      ...
      while (derivedObjects.hasNext()) {
        ComponentRelation derivedRelation = (ComponentRelation) derivedObjects.next();
    ...
    if (_undoEnabled) {
      ...
      if (relation == null) {
        ...
        if (port.getDerivedLevel() == Integer.MAX_VALUE) {
          ...
          if (insertAt != -1) {
            if (port.numLinks() != origNumOutsideLinks) {
              // Enclose in a group to prevent deferral on undo.
              _undoContext.appendUndoMoML("<group><unlink port=\"" + portName + "\" index=\"" + insertAtSpec + "\" /></group>\n");
            }
          } else {
            if (port.numInsideLinks() != origNumInsideLinks) {
              // Enclose in a group to prevent deferral on undo.
              _undoContext.appendUndoMoML("<group><unlink port=\"" + portName + "\" insideIndex=\"" + insertInsideAtSpec + "\" /></group>\n");
            }
          }
        }
      } else {
        ...
        // not derived. If they are both derived, then the link belongs to
        // the class definition and should not be undone in undo.
        if (port.getDerivedLevel() == Integer.MAX_VALUE || relation.getDerivedLevel() == Integer.MAX_VALUE) {
          ...
          if (port.numInsideLinks() != origNumInsideLinks) {
            ...
            _undoContext.appendClosingUndoMoML("<group><unlink port=\"" + portName + "\" insideIndex=\"" + insertInsideAt + "\" /></group>" + "\n");
...
//          }
  ...
  private void _processUnlink(String relation1Name, String relation2Name) throws XmlException, IllegalActionException {
    ...
    // it will not be persistent.
    if (_isLinkInClass(context, relation1, relation2)) {
      throw new IllegalActionException(relation1, relation2, "Cannot unlink relations when both" + " are part of the class definition.");
    }
    ...
    if (_undoEnabled) {
      ...
      if (relation1.getDerivedLevel() == Integer.MAX_VALUE || relation2.getDerivedLevel() == Integer.MAX_VALUE) {
        _undoContext.appendUndoMoML("<link relation1=\"" + relation1Name + "\" relation2=\"" + relation2Name + "\" />\n");
      }
    }
  }
  ...
  private void _processUnlink(String portName, String relationName, String indexSpec, String insideIndexSpec) throws XmlException, IllegalActionException {
    _checkClass(_current, CompositeEntity.class, "Element \"unlink\" found inside an element that " + "is not a CompositeEntity. It is: " + _current);
    ...
    if (countArgs != 1) {
      throw new XmlException("Element unlink requires exactly one of " + "an index, an insideIndex, or a relation.", _currentExternalEntity(),
          _getLineNumber(), _getColumnNumber());
    }
    ...
    if (relationName != null) {
      ...
      // Ensure that derived objects aren't changed.
      if (_isLinkInClass(context, port, relation)) {
        throw new IllegalActionException(port, "Cannot unlink a port from a relation when both" + " are part of the class definition.");
      }
      ...
      // not derived. If they are both derived, then the link belongs to
      // the class definition and should not be recreated in undo.
      if (_undoEnabled && (port.getDerivedLevel() == Integer.MAX_VALUE || relation.getDerivedLevel() == Integer.MAX_VALUE)) {
        ...
        if (index != -1) {
          ...
          _undoContext.appendUndoMoML("<link port=\"" + portName + "\" insertAt=\"" + index + "\" relation=\"" + relationName + "\" />\n");
        } else {
          List insideLinkedRelations = port.insideRelationList();
          index = insideLinkedRelations.indexOf(tmpRelation);

          // Linked on the inside.
          _undoContext.appendUndoMoML("<link port=\"" + portName + "\" insertInsideAt=\"" + index + "\" relation=\"" + relationName + "\" />\n");
        }
      }
      ...
      if (relationList.size() <= index) {
        throw new IllegalActionException(port, "Cannot unlink index " + indexSpec + ", because there is no such link.");
      }
      ...
      if (_undoEnabled) {
        ...
        if (port.getDerivedLevel() == Integer.MAX_VALUE || (r != null && r.getDerivedLevel() == Integer.MAX_VALUE)) {
          ...
          _undoContext.appendUndoMoML("<link port=\"" + portName + "\" insertAt=\"" + indexSpec + "\" ");

          // Only need to specify the relation if there was
          // a relation at that index. Otherwise a null
          // link is inserted
          if (r != null) {
            _undoContext.appendUndoMoML("relation=\"" + r.getName(context) + "\" ");
          }
          _undoContext.appendUndoMoML(" />\n");
        }
      }
      ...
      if (_undoEnabled) {
        ...
        if (port.getDerivedLevel() == Integer.MAX_VALUE || (r != null && r.getDerivedLevel() == Integer.MAX_VALUE)) {
          // FIXME: need to worry about vertex?
          _undoContext.appendUndoMoML("<link port=\"" + portName + "\" insertInsideAt=\"" + index + "\" ");
  ...
  /**
   * Reset the undo information to give a fresh setup for the next incremental change. NOTE: this resets all the undo information except for the
  ...
  /**
   * Given a name that is either absolute (with a leading period) or relative to _current, find an attribute with that name. The attribute is required to be
   * contained (deeply) by the current environment, or an XmlException will be thrown.
   * 
  ...
  private Attribute _searchForAttribute(String name) throws XmlException {
    ...
    if (result == null) {
      throw new XmlException("No such property: " + name + " in " + currentName, _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
  ...
  /**
   * Search for a class definition in the current context or anywhere above it in the hierarchy. If a instance of a MoML class with a matching name and source
   * is found, then return it. Otherwise, return null.
   * 
  ...
  /**
   * Given a name that is either absolute (with a leading period) or relative to the specified context, find a component entity with that name. Return null if
   * it is not found.
   * 
   * @param name The name of the entity.
   * @param context The context in which to search.
   * @return An entity with the specified name, or null if none is found.
   * @exception XmlException If the name refers to an entity in an innappropriate context or if the context is not an instance of CompositeEntity.
   */
  private ComponentEntity _searchForEntity(String name, NamedObj context) throws XmlException {
    ...
    if (name.startsWith(".")) {
      ...
      if ((_toplevel != null) && _toplevel instanceof ComponentEntity && topLevelName.equals(_toplevel.getName())) {
        if (nextPeriod < 1) {
          /*
           * NOTE: This is too restrictive. With an absolute name, there is no reason to disallow setting the context. This is useful in particular when
           * deleting ports to make sure undo works. The undo code has to execute in a context that may be higher than that in which the port was deleted in
           * order for the connections to be re-established.
           */
          /*
           * if (context != null && context != _toplevel) { throw new XmlException( "Reference to an existing entity: " + _toplevel.getFullName() +
           * " in an inappropriate context: " + context.getFullName(), _currentExternalEntity(), _getLineNumber(), _getColumnNumber()); }
           */
          return (ComponentEntity) _toplevel;
        } else {
          if (name.length() > (nextPeriod + 1)) {
            ComponentEntity result = ((CompositeEntity) _toplevel).getEntity(name.substring(nextPeriod + 1));
            ...
            if (result != null) {
               ...
               */
              /*
               * if (context != null && !context.deepContains(result)) { throw new XmlException( "Reference to an existing entity: " + result.getFullName() +
               * " in an inappropriate context: " + context.getFullName(), _currentExternalEntity(), _getLineNumber(), _getColumnNumber()); }
               */
              return result;
            }
  ...
  /**
   * Given a name that is either absolute (with a leading period) or relative to _current, find a port with that name. The port is required to be contained
   * (deeply) by the current environment, or an XmlException will be thrown.
   * 
  ...
  private Port _searchForPort(String name) throws XmlException {
    ...
    if (result == null) {
      throw new XmlException("No such port: " + name + " in " + topLevelName, _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
  ...
  /**
   * Given a name that is either absolute (with a leading period) or relative to _current, find a relation with that name. The relation is required to be
   * contained (deeply) by the current environment, or an XmlException will be thrown.
   * 
  ...
  private ComponentRelation _searchForRelation(String name) throws XmlException {
    ...
    if (result == null) {
      throw new XmlException("No such relation: " + name + " in " + topLevelName, _currentExternalEntity(), _getLineNumber(), _getColumnNumber());
    }
