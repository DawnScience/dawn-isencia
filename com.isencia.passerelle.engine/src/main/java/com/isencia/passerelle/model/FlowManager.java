 ...
 * </ul>
 * <p>
 * The <code>readMoml(URL)</code> can interact with the REST web services of a
 * Passerelle Manager instance. The URL should then identify a flow resource on
 * the Passerelle Manager. To bootstrap this,
 * <code>getFlowsFromResourceLocation(URL)</code> allows to retrieve the
 * complete list of all known flows on the Passerelle Manager instance
...
public class FlowManager {
  ...
  public static FlowManager getDefault() {
	 ...
	 * FlowHandle instances for them. <br/>
	 * From these FlowHandles, the full flow can be constructed. <br/>
	 * In this way, we can implement a lazy-loading approach for Flows.
	 * <p>
	 * If the URL points to a local location, it should be a folder on a local
	 * disk. Alternatively it can be a REST URL pointing to a Passerelle
	 * Manager. A sample REST URL is in the form of :<br/>
	 * http://localhost:8080/passerelle-manager/bridge/PasserelleManagerService/
	 * V1.0/jobs/
	 * </p>
	 * 
	...
	/**
	 * Get remote execution traces of the given flow. Remark that this may also
	 * update the flow execution status info in the handle if the remote server
	 * responds with status "inactive". From that moment on, no more execution
	 * traces can be requested for this handle!
	 * 
	 * @param fHandle
	 * @throws PasserelleException
  ...
  public static Flow readMoml(URL xmlFile, Workspace workspace, VersionSpecification versionSpec, ClassLoader classLoader) throws Exception {
	 ...
	 * should point to a local file). For remotely managed flows, updates the
	 * flow on the remote Passerelle Manager. Then the url parameter is ignored.
	 * 
	 * @param flow
	 * @param url
	 * @return the updated/saved flow (if it's a remote one, then with updated
	 *         FlowHandle)
	 * @throws IOException
	 * @throws PasserelleException
	 */
	...
	/**
	 * Execute the given flow on the Passerelle Manager server instance, from
	 * which it was previously obtained. <br/>
	 * Locally constructed Flows can not be launched on a server!
	 * <p>
	 * If the <code>updatedParams</code> is non-empty, a new model revision is
	 * created and activated on the server first, with the updated parameters.
	 * Then this new revision is launched.
	 * </p>
	 * <p>
	 * Parameters should be identified by their full names, but optionally the
	 * flow's name can be chopped off. E.g. .helloWorldModel.Constant.value or
	 * Constant.value
	 * </p>
	 * <p>
	 * For parameters inside submodels, the submodel-hierarchy must still be
	 * present. E.g. .helloWorldModel.submodel.subSubmodel.Constant.value or
	 * submodel.subSubmodel.Constant.value
	 * </p>
	 * <p>
	 * Remark that when the model name is present, there's an extra spurious '.'
	 * at the beginning, which is not assumed to be present when the model name
	 * is absent!
	 * </p>
	 * 
	 * @param flow
	 *            a flow with a authorative resource URL on a Passerelle Manager
	 *            server
	 * @param updatedParams
	 ...
	 *            : if null : return locally executing flows, otherwise should
	 *            have http(s) as protocol and should identify a Passerelle
	 *            Manager web service entry point
	 * 
	 * @return
	 * @throws PasserelleException
	 */
