...
/**
 * A customization of the std Ptolemy Ports, that includes automatic conversions
 * between Passerelle messages and plain Ptolemy tokens (to integrate with plain
...
public class Port extends TypedIOPort {
    ...
    /**
     * For an input port, when not null, this buffer is used
     * as a common queue for each Receiver on each input channel.
     * <p>
     * This is an experimental feature to avoid the usage of extra
     * PortHandler.ChannelHandler threads i.c.o. multi-channel input ports.
     * If this mechanism is proven to work it could even be extended to support
     * a common buffer across multiple PUSH-mode input ports, also
     * making the usage of extra handler threads redundant in such situations.
     * </p>
     */
    private MessageBuffer buffer;

	/**
	 * A detailed type definition of the expected content of incoming Passerelle
	 * messages. This is used to provide automated conversion of body contents
	 * of Passerelle messages. If it is left null, no conversion is attempted.
	 */
	...
	/**
	 * REMARK : need to put this one as public, just for Ptolemy's sake...
	 * During parsing of moml files, Ptolemy sometimes wants to create the Ports
	 * directly itself. BLAST!
	 * 
	 * @param container
	 * @param name
	...
	/**
	 * 
	 * @return Returns the expectedMessageContentType, the expected type of the
	 *         body content of incoming Passerelle msgs.
	 */
	...
	/**
	 * @param expectedMessageContentTypse
	 *            The expectedMessageContentType to set, the expected type of
	 *            the body content of incoming Passerelle msgs.
	 */
	...
	/**
	 * Send a token to all connected receivers. Tokens are in general immutable,
	 * so each receiver is given a reference to the same token and no clones are
	 * made. The transfer is accomplished by calling getRemoteReceivers() to
	 * determine the number of channels with valid receivers and then calling
	 * send() on the appropriate channels. It would probably be faster to call
	 * put() directly on the receivers. If there are no destination receivers,
	 * then nothing is sent. If the port is not connected to anything, or
	 * receivers have not been created in the remote port, then just return.
	 * <p>
	 * Some of this method is read-synchronized on the workspace. Since it is
	 * possible for a thread to block while executing a put, it is important
	 * that the thread does not hold read access on the workspace when it is
	 * blocked. Thus this method releases read access on the workspace before
	 * calling put.
	 * 
	 * @param token
	 *            The token to send
	 ...
	 * sent.
	 * <p>
	 * Tokens are in general immutable, so each receiver is given a reference to
	 * the same token and no clones are made. If the port is not connected to
	 * anything, or receivers have not been created in the remote port, or the
	 * channel index is out of range, or the port is not an output port, then
	 * just silently return. This behavior makes it easy to leave output ports
	 * unconnected when you are not interested in the output. The transfer is
	 * accomplished by calling the vectorized put() method of the remote
	 * receivers. If the port is not connected to anything, or receivers have
	 * not been created in the remote port, then just return.
	 * <p>
	 * Some of this method is read-synchronized on the workspace. Since it is
	 * possible for a thread to block while executing a put, it is important
	 * that the thread does not hold read access on the workspace when it is
	 ...
	 * the token if necessary. If the port is not connected to anything, or
	 * receivers have not been created in the remote port, or the channel index
	 * is out of range, or the port is not an output port, then just silently
	 * return. This behavior makes it easy to leave output ports unconnected
	 * when you are not interested in the output. If the type of the specified
	 * token is the type of this port, or the token can be converted to that
	 * type losslessly, the token is sent to all receivers connected to the
	 * specified channel. Otherwise, IllegalActionException is thrown. Before
	 * putting the token into the destination receivers, this method also checks
	 * the type of the remote input port, and converts the token if necessary.
	 * The conversion is done by calling the convert() method of the type of the
	 * remote input port.
	 * <p>
	 * Some of this method is read-synchronized on the workspace. Since it is
	 * possible for a thread to block while executing a put, it is important
	...
	/**
	 * Send the specified portion of a token array to all receivers connected to
	 * the specified channel, checking the type and converting the token if
	 * necessary. The first <i>vectorLength</i> tokens of the token array are
	 * sent. If the port is not connected to anything, or receivers have not
	 * been created in the remote port, or the channel index is out of range, or
	 * the port is not an output port, then just silently return. This behavior
	 * makes it easy to leave output ports unconnected when you are not
	 * interested in the output.
	 * <p>
	 * To improve efficiency for the common case where the type of the tokens to
	 * send matches the type of this port and all connected ports, this method
	 * assumes that all of the tokens in the specified portion of the token
	 * array are of the same type. If this is not the case, then the
	 * non-vectorized send() method should be used instead. The implementation
	 * only actually checks the type of the first token in the array, and then
	 * assumes that the remaining tokens are of the same type.
	 * <p>
	 * If the type of the tokens in the specified portion of the token array is
	 * the type of this port, or the tokens in the specified portion of the
	 * token array can be converted to that type losslessly, the tokens in the
	 * specified portion of the token array are sent to all receivers connected
	 * to the specified channel. Otherwise, IllegalActionException is thrown.
	 * Before putting the tokens in the specified portion of the token array
	 * into the destination receivers, this method also checks the type of the
	 * remote input port, and converts the tokens if necessary. The conversion
	 * is done by calling the convert() method of the type of the remote input
	 * port.
	 * <p>
	 * Some of this method is read-synchronized on the workspace. Since it is
	 * possible for a thread to block while executing a put, it is important
	 ...
	 * @exception IllegalActionException
	 *                If the tokens to be sent cannot be converted to the type
	 *                of this port, or if the <i>vectorLength</i> argument is
	 *                greater than the length of the <i>tokenArray</i>
	 *                argument.
	 */
	public void send(int channelIndex, Token[] tokenArray, int vectorLength) throws IllegalActionException, NoRoomException {
		if (logger.isTraceEnabled()) {
			logger
					.trace("send(array) - entry : channel : " + channelIndex + " token : " + tokenArray + " length : "
							+ vectorLength);
		}
	...
	/**
	 * 
	 * @return the port's message buffer. 
	 * If this is not-null, and the port is used in a Passerelle model (i.e. with a Passerelle Director),
	 * all the port's receivers should directly feed their incoming tokens into this buffer.
	 * <br/>
	 * I.e. there is no need to provide separate PortHandler.ChannelHandler threads anymore on each
	 * input channel, but the actor's process thread can be directly used to control the (blocking) get()
	 * on this port. 
	 */
	...
	/**
	 * Get a token from the specified channel. If the channel has a group with
	 * more than one receiver (something that is possible if this is a
	 * transparent port), then this method calls get() on all receivers, but
	 * returns only the first non-null token returned by these calls. Normally
	 * this method is not used on transparent ports. If there is no token to
	 * return, then throw an exception.
	 * <p>
	 * Some of this method is read-synchronized on the workspace. Since it is
	 * possible for a thread to block while executing a get, it is important
	 * that the thread does not hold read access on the workspace when it is
	 * blocked. Thus this method releases read access on the workspace before
	 * calling get().
	 * 
	 ...
	 * @exception IllegalActionException
	 *                If there is no director, and hence no receivers have been
	 *                created, if the port is not an input port, or if the
		...
		try {
			...
			if (channelIndex >= localReceivers.length) {
				if (!isInput()) {
					...
					throw new IllegalActionException(this, "Channel index " + channelIndex
	 ...
	 * <i>channelIndex</i> specifies the channel and the parameter
	 * <i>vectorLength</i> specifies the number of valid tokens to get in the
	 * returned array. The length of the returned array can be greater than the
	 * specified vector length, in which case, only the first <i>vectorLength</i>
	 * elements are guaranteed to be valid.
	 * <p>
	 * If the channel has a group with more than one receiver (something that is
	 * possible if this is a transparent port), then this method calls get() on
	 * all receivers, but returns only the first non-null token returned by
	 * these calls. Normally this method is not used on transparent ports. If
	 * there are not enough tokens to fill the array, then throw an exception.
	 * <p>
	 * Some of this method is read-synchronized on the workspace. Since it is
	 * possible for a thread to block while executing a get, it is important
	 * that the thread does not hold read access on the workspace when it is
	 ...
	 *            The channel index.
	 * @param vectorLength
	 *            The number of valid tokens to get in the returned array.
...
//			return result;
		...
		if (localActor instanceof Actor) {
			...
			if (!(farActor instanceof Actor) || !PasserelleType.PASSERELLE_MSG_TYPE.equals(farPort.getType())) {
				...
				try {
					...
					converted = TypeConversionChain.getInstance().convertPasserelleTokenToPtolemyToken((PasserelleToken) token,
							farPort.getType());
				} catch (UnsupportedOperationException e) {
					// do nothing
					// the user will get loads of Ptolemy errors anyway
	...
	/**
	 * Custom initialization method, called by the enclosing actor, 
				...
				for (int i = 0; i < receivers.length; i++) {
					...
					for (int j = 0; j < receivers2.length; j++) {
						...
						if(receiver instanceof BlockingQueueReceiver) {
							((BlockingQueueReceiver)receiver).setMessagBuffer(_msgBfr);
						}
					}
				}
			} else {
