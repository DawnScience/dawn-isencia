...
public class BlockingQueueReceiver extends QueueReceiver
     ...
     * @param queue
     */
    public void setMessagBuffer(MessageBuffer buffer) {
    	this.buffer = buffer;   
    	if(buffer!=null)
    ...
    /**
     * Get the next token from the shared queue,
     * or from the private queue when no shared queue was set.
     * If this receiver has been terminated (e.g. via requestFinish()),
     * and no tokens are queued anymore, null is returned.
     * If the receiver is not terminated, this method blocks till a token
     * has been received in the queue.
     *
     * @return the next token received, or null when the receiver has finished
     * and all tokens have been retrieved already.
     */
    public Token get() {
    ...
    /**
     * Passerelle BlockingQueueReceiver always has place for more.
     * No Max capacity, just a warning level notification.
     * @return true
     */
	@Override
	...
	/**
     * Store the token in the queue.
     * If the size warning threshold has been reached, a warning msg is logged.
     *
     * @param token 
     */
	@Override
...
//				int i = getMyIndex();
     ...
     * From then, no new tokens are stored and after all tokens have been retrieved,
     * a null token is returned as an indicator that this receiver is finished.
     */
