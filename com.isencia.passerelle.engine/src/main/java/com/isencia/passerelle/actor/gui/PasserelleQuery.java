...
public class PasserelleQuery extends ptolemy.actor.gui.PtolemyQuery implements IPasserelleQuery, IPasserelleComponent {
	 ...
	 * deeply contains all attributes that are attached to query entries. Otherwise, the change requests might get
	 * queued with a handler that has nothing to do with the attributes. The handler is also used to report errors.
	 * 
	 * @param handler
	 *            The change handler.
	 */
...
//		System.out.println("value changed for "+attribute.getFullName());
	...
	/**
	 * Add a new entry to this query that represents the given attribute. The name of the entry will be set to the name
	 * of the attribute, and the attribute will be attached to the entry, so that if the attribute is updated, then the
	 * entry is updated. If the attribute contains an instance of ParameterEditorStyle, then defer to the style to
	 * create the entry, otherwise just create a default entry. The style used in a default entry depends on the class
	 * of the attribute and on its declared type, but defaults to a one-line entry if there is no obviously better
	 * style. Only the first style that is found is used to create an entry.
	 * 
	 * @param attribute
		...
		if (!foundStyle) {
            ...
            if (!allowFiles && !allowDirectories) {
              ...
              return;
					...
					addDatePicker(name, label, ((DateTimeParameter) attribute).getDateValue(), Color.ORANGE,
					...
					addChoice(name, label, castAttribute.getChoices(), castAttribute.getExpression(), true,
							preferredBackgroundColor(attribute), preferredForegroundColor(attribute));
					attachParameter(attribute, name);
					foundStyle = true;
	...
	public void addFileChooser(String name, String label, String defaultName, URI base, File startingDirectory,
			FileFilter fileFilter, Color background, Color foreground) {

		JLabel lbl = new JLabel(label + ": ");
		lbl.setBackground(_background);
		QueryFileChooser fileChooser = new QueryFileChooser(name, defaultName, base, startingDirectory, fileFilter,
	...
	public final void set(String name, String value) throws NoSuchElementException, IllegalArgumentException {
		try {
			...
			// eat it, it's almost always caused by our overwritten/added param cfg panels (file chooser, colour picker etc)
			// that are unknown to Ptolemy's Query class.
			// TODO find out a neater way to extend Ptolemy query without needing to duplicate their inner classes etc.
		}
	}
	...
	/**
	 * Notify the listener that a change attempted by the change handler has resulted in an exception. This method
	 * brings up a new dialog to prompt the user for a corrected entry. If the user hits the cancel button, then the
	 * attribute is reverted to its original value.
	 * 
	 * @param change
			...
			if (_isOpenErrorWindow) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						...
						_dialog = new ComponentDialog(JOptionPane.getFrameForComponent(PasserelleQuery.this), "Error",
								_query, null);
	...
	class QueryDatePicker extends DateTimeSelector {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if (allowChangeNotifications
					&& (evt.getPropertyName().equals("closeMe") || evt.getPropertyName().equals("time"))) {
