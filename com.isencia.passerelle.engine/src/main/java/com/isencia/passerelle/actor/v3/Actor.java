...
/**
 * <p>
 * A new uniform Actor API that provides a better encapsulation of the specific
 * treatment needed for the different types of port interactions (input/output,
 * pull/push, ...).
 * </p>
 * <p>
 * Actor implementations should now register any input ports with the desired
 * mode:
 * <ul>
 * <li> PortMode.PULL : for blocking ports, where the actor will try to get a
 * message, and remain blocked, waiting for a message to arrive.
 * <li> PortMode.PUSH : for non-blocking ports, where the actor is notified when
 * a message arrives
 * </ul>
 * </p>
 * <p>
 * Secondly, an actor must no longer implement any (derivatives of)
 * preFire/fire/postFire. Instead a single new method process(...) is provided.
 * <br>
 * From the ProcessRequest parameter, the actor implementation can ask for the
 * last message received on a given input port.
 * <ul>
 * <li>For PULL (blocking) ports, each process() invocation will have a new
 * message available (as the actor blocks until a new message has arrived).
 * <li>For PUSH (non-blocking) ports, it's possible that no message is
 * available for the port. In that case, the last previously received message is
 * given, or null if no message has ever been received on it.
 * </ul>
 * The actor can also obtain the iteration counter, for which the ProcessRequest
 * has been constructed. <br>
 * If the actor wants to send outgoing messages, it should pass them with the
 * desired output port into the ProcessResponse. <br>
 * The actor has the option to send several outgoing messages grouped in a
 * sequence, or as individual, "unrelated" messages.
 * </p>
 * 
...
public abstract class Actor extends com.isencia.passerelle.actor.Actor {
	...
	/**
	 * The bridge between this actor and its ports that are configured in PUSH
	 * mode.
	 * 
	 * @author erwin
	 * 
	...
	private static class MsgListener extends PortListenerAdapter {
		...
		public void tokenReceived() {
			...
			try {
				ManagedMessage msg = MessageHelper
						.getMessageFromToken(selectToken);
				int i = getMyIndex();
				MessageInputContext ctxt = new MessageInputContext(i, handler
						.getName(), msg);
		...
		/**
		 * 
		 * @return the index of this listener (or its port) in the list of known
		 *         pushing ports.
		 */
	 ...
	 */
	public Actor(CompositeEntity container, String name)
	...
	protected void doFire() throws ProcessingException {
		...
		if (!isSource) {
			...
			for (int i = 0; i < blockingInputHandlers.size(); i++) {
				...
				if (!blockingInputFinishRequests.get(i).booleanValue()) {
					...
					if (msg == null) {
						...
						if (logger.isDebugEnabled())
							logger.debug(getInfo() + " doFire() - msg "
									+ msg.getID() + " received on port "
									+ handler.getName());
					}
				}
			...
			Collection<MessageInputContext> pushedMsgs = pushedMessages
					.values();
		...
		if (isSource || req.hasSomethingToProcess()) {
			...
			if (outputs != null && outputs.length > 0) {
				...
				for (int i = 0; i < outputs.length; i++) {
					...
					try {
						ManagedMessage msgInSeq = MessageFactory.getInstance().createMessageCopyInSequence(
										context.getMessage(), seqID, new Long(i), isLastMsg);
						sendOutputMsg(context.getPort(), msgInSeq);
					} catch (MessageException e) {
						throw new ProcessingException("Error creating output sequence msg for msg "
										+ context.getMessage().getID(), context.getMessage(), e);
					}
				}
			}
		}
	...
	protected final boolean areAllInputsFinished() {
		...
		for (int i = 0; i < blockingInputFinishRequests.size(); ++i) {
			result = result
					&& blockingInputFinishRequests.get(i).booleanValue();
		}
	 ...
	 */
	protected abstract void process(ActorContext ctxt, ProcessRequest request,
			ProcessResponse response) throws ProcessingException;

	/**
	 * <p>
	 * Method that should be overridden for actors that need to be 
	 * able to validate their state before processing a next fire-iteration.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param ctxt
	 * @param request contains all messages received on the actor's input ports
	 ...
	 * a validation of its state and incoming request for each iteration.
	 * <br>
