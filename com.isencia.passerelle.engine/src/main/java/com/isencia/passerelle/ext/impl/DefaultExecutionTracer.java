 ...
 * a "trace" logger, on the INFO level.
 * 
 * Depending on the execution environment, different results can be obtained by
 * modifying the logging properties. E.g. trace output can be sent to a log
 * file, or to a database etc.
 * 
 * <code>ExecutionTracers</code> are typically used through the {@link ExecutionTracerService}.
 * In order to ensure only one instance of a {@link DefaultExecutionTracer} is active
 * inside the {@link ExecutionTracerService}, the <code>equals()</code> and <code>hashCode()</code>
 * methods have been defined to just check on the class name being equal.
 * 
 * @author erwin
 * 
 */
