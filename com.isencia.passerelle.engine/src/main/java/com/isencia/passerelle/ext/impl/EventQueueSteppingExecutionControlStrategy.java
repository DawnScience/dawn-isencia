...
/**
 * An execution controller that lets a model execute in stepping mode.
 * Each step allows one actor to do a fire() iteration.
 * A next step is triggered by a stepping request event.
 * Such requests are maintained in an internal queue, from which they are taken
 * for each actor requesting a next iteration.
 * <br>
 * Furthermore it allows to stop the stepping mode and continue 
 * in plain running mode.
 * 
 * @author erwin
 *
 */
public class EventQueueSteppingExecutionControlStrategy extends Attribute implements
